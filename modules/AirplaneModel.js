...
export class AirplaneModel {
    constructor(scene) {
        ...
        this.elevatorGroup = null; // 升降舵组（水平尾翼）
    ...
    createTail() {
        ...
        // 创建水平尾翼（升降舵组）
        this.createElevator(wingMaterial);

        // 垂直尾翼和方向舵
        this.createRudder(wingMaterial);
    }

    createElevator(wingMaterial) {
        this.elevatorGroup = new THREE.Group();

        // 主水平尾翼
        const tailWingGeometry = new THREE.BoxGeometry(2, 0.2, 2.5);
        const tailWing = new THREE.Mesh(tailWingGeometry, wingMaterial);
        tailWing.position.set(0, 0, 0);
        tailWing.castShadow = true;
        this.elevatorGroup.add(tailWing);

        // 可动的升降舵部分（红色，便于观察）
        const elevatorGeometry = new THREE.BoxGeometry(1, 0.15, 2.2);
        const elevatorMaterial = new THREE.MeshPhongMaterial({ color: 0xff3333 }); // 红色便于观察
        const elevator = new THREE.Mesh(elevatorGeometry, elevatorMaterial);
        elevator.position.set(-1, 0, 0); // 在尾翼后部
        elevator.castShadow = true;
        this.elevatorGroup.add(elevator);

        // 设置升降舵组的位置
        this.elevatorGroup.position.x = -4;
        this.airplane.add(this.elevatorGroup);
    }
